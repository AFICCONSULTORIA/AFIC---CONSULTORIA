...
export const CRMAdmin = () => {
  ...
  const questionMap = {
    renda_atual: "Renda Mensal Atual",
    renda_sonho: "Renda dos Sonhos Pretendida",
  ...
  const answerMap = {
    ...
    "pouco": "Ganha pouco, só rico investe. / Tenho pouco tempo, quero algo passivo.",
    ...
    "medio": "Aceito esperar / Consigo dedicar 1-2 horas semanais.",
  ...
  const getAnswerText = (key, val) => {
    if (!val) return null;
    if (key === 'renda_atual' || key === 'renda_sonho') return formatBRL(val);
  ...
  const formatBRL = (val) => {
    ...
    // Remove caracteres não numéricos para verificar se é um valor puro
    const numeric = val.replace(/\D/g, '');
    if (!numeric) return val;
    
    // Se o valor for muito grande (ex: centavos), tratamos, caso contrário formatamos o número bruto
  ...
  const filteredLeads = leads.filter(lead => {
    ...
    const matchesStatus = statusFilter === 'all' || (lead.status || 'novo') === statusFilter;
        ...
        <div className="bg-white dark:bg-[#0a2540] border border-gray-200 dark:border-gray-800 rounded-lg px-4 py-2 flex gap-4 text-sm font-medium shadow-sm">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-xl font-bold text-blue-500">
              {leads.filter(l => (l.status || 'novo') === 'novo').length}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative">
          <input 
            ...
            className="w-full bg-white dark:bg-[#0a2540] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#cda434]/20 focus:border-[#cda434] outline-none transition-all"
          />
        </div>
        <div className="w-full md:w-48">
          <select
            ...
            className="w-full bg-white dark:bg-[#0a2540] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#cda434]/20 focus:border-[#cda434] outline-none transition-all cursor-pointer"
          >
            ...
            {filteredLeads.map((lead) => {
              ...
              return (
                <div key={lead.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-[#0c2a4a]">
                    ...
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3">
                          <h4 className="font-bold text-gray-900 dark:text-white truncate">{lead.nome}</h4>
                          <div className="flex gap-2 shrink-0">
                             {lead.renda_atual && (
                               <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-100 dark:border-amber-800/50">
                                 Renda: {formatBRL(lead.renda_atual)}
                               </span>
                             )}
                          </div>
                        </div>
                    ...
                    <div className="flex items-center gap-6 justify-between md:justify-end">
                      ...
                      <div className="relative group" onClick={e => e.stopPropagation()}>
                        <select 
                          ...
                          className={`appearance-none cursor-pointer pl-3 pr-8 py-1.5 rounded-full text-xs font-bold border outline-none transition-colors
                  ...
                  {isExpanded && (
                    <div className="px-6 py-5 bg-gray-50/50 dark:bg-[#071d36] border-t border-gray-100 dark:border-gray-800 animate-fade-in">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
                        {Object.entries(questionMap).map(([key, question]) => (
                          <div key={key} className="bg-white dark:bg-[#0a2540] p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm hover:border-[#cda434]/30 transition-colors">
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-tight mb-2">{question}</p>
                            <p className="text-sm font-semibold text-gray-900 dark:text-gray-200 leading-snug">
                      ...
                      <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800 flex justify-end">
                        <a 
                          ...
                          className="flex items-center gap-2 bg-[#25D366] text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-[#20bd5a] hover:scale-105 transition-all shadow-lg shadow-green-500/20"
                        >
