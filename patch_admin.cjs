...
const startIndex = content.indexOf('window.loadAnalisePerfilResponses = async function()');
const endIndex = content.indexOf('window.showAnalisePerfilDetail = function(r)');

if (startIndex !== -1 && endIndex !== -1) {
    const newContent = content.substring(0, startIndex) + updatedLoadLogic + '\n\n' + content.substring(endIndex);
    fs.writeFileSync('app.js', newContent, 'utf8');
    console.log('App.js admin list updated successfully using indexOf');
} else {
    console.error('Could not find start or end function');
